...
STATE_CLEAR, STATE_EDITOR, STATE_SETTINGS = 문자열 상수
...
class SaveManager:
    ...
    # data 구조: {"unlocked": int, "records": {level_idx: {moves,time,stars,date}}, "settings": {...}}
```
...
class Renderer:
    ...
    def render_level(screen, engine: GameEngine, camera_offset, anim_state)
    def render_hud(screen, engine, level_info: dict)
```

### ParticleSystem (particles.py)
```
class Particle: x,y, vx,vy, color, life, size
class ParticleSystem:
    def emit(x, y, count, colors, speed_range, life_range)
    def update(dt)
    def draw(screen)
```

### 상태 머신 — 모든 State 클래스 (ui.py + main.py)
```
class State:
    def enter(context: dict)              # 상태 진입, context로 공유 데이터 전달
    def exit()
...
ClearState, EditorState, SettingsState
```
...
sound = SoundManager()
save = SaveManager()
context = {"assets": assets, "sound": sound, "save": save, ...}
states = {이름: State인스턴스, ...}
...
- Z키: Undo (무제한)
- Y키: Redo
- 스택에 (player_pos, boxes_frozenset) 저장
...
모든 스프라이트는 pygame.Surface(TILE_SIZE×TILE_SIZE)에 pygame.draw 함수(rect, circle, polygon, line)로 도형을 조합하여 그린다. set_at()으로 개별 픽셀 찍기 금지. 효율적으로 그려야 코드량이 관리 가능하다.

**벽**: 진갈색 배경 + 밝은 벽돌 라인 격자 + 상단/좌측 하이라이트 + 하단/우측 셰도우
**바닥**: 베이지 배경 + 연한 격자 라인
**목표**: 바닥 위에 빨간 다이아몬드 또는 X 마크 (tick 기반 펄스: alpha 또는 크기 변화)
...
## 5. 내장 레벨 데이터 (level_data.py에 그대로 사용할 것)

아래 20개 레벨을 정확히 이 순서와 데이터로 `LEVELS` 리스트에 넣어라.
각 레벨은 `{"name": str, "difficulty": str, "par": int, "data": str}` 형식이다.
레벨 data 문자열은 아래 텍스트를 **한 글자도 바꾸지 말고** 그대로 사용해야 한다.
...
**Level 11 — "Three in Row" / Medium / par=30**
```
 #######
 #     #
## ###.#
...
**Level 12 — "Storage" / Medium / par=35**
```
  ######
  #    #
### ## ##
...
**Level 14 — "Warehouse" / Medium / par=38**
```
  #####
###   #
# $ # ##
...
**Level 15 — "Puzzle Box" / Medium / par=45**
```
#######
#  .  #
# #$# #
...
**Level 17 — "Double Cross" / Hard / par=55**
```
  #####
###   ##
#  $   #
...
**Level 19 — "Labyrinth" / Hard / par=65**
```
  #######
  #  .  #
 ## #.# #
...
**Level 20 — "Grand Finale" / Hard / par=75**
```
 ########
 #  ... #
## #.#  #
...
numpy로 사운드 파형을 생성. numpy import 실패 시 enabled=False로 무음 동작 (폴백 코드 불필요, try/except 한 줄이면 됨).

사운드 생성법:
- sample_rate = 22050
- **중요**: `pygame.init()` 호출 전에 `pygame.mixer.pre_init(frequency=22050, size=-16, channels=1)` 호출 (init 순서 충돌 방지)
...
- 좌측 80px: 타일 팔레트 (세로 배열)
- 중앙: 편집 캔버스 (그리드 + 마우스 호버 프리뷰)
- 하단 30px: 상태 텍스트 ("Boxes: 3 / Goals: 3 ✓")
...
**TITLE**
- 상단: "SOKOBAN" 큰 텍스트 (폰트 크기 72)
- 중앙: 메뉴 항목 5개 세로 배열 — Play, Level Select, Editor, Settings, Quit
- 선택 표시: 노란색 + 좌측 ">" 표시
- 조작: 상/하 방향키로 선택 이동, Enter로 진입
- Play 선택 시: 마지막 미클리어 레벨 또는 레벨 1로 시작

**LEVEL_SELECT**
- 4열 × 5행 그리드 레벨 버튼 (20개)
- 각 버튼: 레벨 번호, 잠금 시 자물쇠 아이콘(사각형+원), 클리어 시 별점(★ 문자)
- 최고 기록 이동 횟수 표시
- 방향키 커서 이동, Enter 선택, ESC 뒤로
- 잠금 해제 규칙: 이전 레벨 클리어 시 다음 해제 (레벨 1은 항상 해제)
...
- 이동 시 lerp 보간 애니메이션 (MOVE_DURATION초)
  - 보간 중 입력은 큐잉하지 말고 무시
  - 보간 상태: moving=True, anim_progress 0→1, 시작/끝 위치 저장
- 클리어 감지 시 → CLEAR 상태로 전환
...
**CLEAR**
- "Level Clear!" 큰 텍스트
- Moves: {n} / Time: {MM:SS} / Stars: ★★★ (또는 ★★, ★)
- 신기록이면 "New Record!" 텍스트 깜빡임
- 컨페티 파티클 이펙트 (다색, 50~100개, 중력 낙하)
- "Next Level" / "Level Select" 버튼
- 마지막 레벨이면 "Next Level" 대신 "Congratulations!" 표시
...
### 8.3 화면 전환
- 모든 전환에 페이드 아웃(검은색, 200ms) → 페이드 인(200ms) 적용
- main.py에서 전환 시 fade 상태 관리
...
```
- 첫 실행 시 data/ 디렉토리와 save.json 자동 생성 (os.makedirs)
- **파일 경로는 main.py 기준 상대 경로가 아닌, 스크립트 위치 기준**: `os.path.join(os.path.dirname(os.path.abspath(__file__)), '..', 'data')` 식으로 구성. 또는 main.py에서 `os.chdir(os.path.dirname(os.path.abspath(__file__)))` 를 초기에 호출하여 CWD를 고정.
- 레벨 클리어 시 자동 저장 (기존 기록보다 좋을 때만 갱신)
- unlocked: 잠금 해제된 최고 레벨 인덱스
- records의 키는 레벨 인덱스(문자열), 값은 {moves, time, stars, date}
...
class ParticleSystem:
    ...
    def emit(x, y, count, colors, speed_range, life_range, size_range)
    def update(dt)   # 위치 += 속도*dt, vy += gravity*dt, life -= dt, 수명 다하면 제거
    def draw(screen) # alpha = life/max_life * 255, pygame.draw.circle
```

사용처:
- 박스→목표 도착: 해당 타일 중심에서 금색 스파클 6개 (속도 느림, 수명 0.5초)
- 레벨 클리어: 화면 상단에서 다색 컨페티 80개 (속도 빠름, 수명 2초, 중력 적용)
...
3. **레벨 데이터 변조 금지**. 섹션 5의 레벨 문자열을 한 글자도 바꾸지 말 것.
4. **set_at() 금지**. 스프라이트는 pygame.draw 함수(rect, circle, polygon, line, ellipse)로만 그릴 것.
5. **실행 즉시 동작**. python main.py로 에러 없이 즉시 타이틀 화면이 나와야 함.
6. **영문 UI만**. 화면에 표시되는 모든 텍스트는 영문. 주석만 한글 허용.
7. **각 파일을 빠짐없이 완전하게 작성**. 파일 목록: main.py, game/__init__.py, game/constants.py, game/assets.py, game/level.py, game/level_data.py, game/engine.py, game/renderer.py, game/editor.py, game/save_manager.py, game/sound.py, game/ui.py, game/particles.py — 총 13개 파일.
...
각 단계마다 0.3초 대기 후 `pygame.image.save(screen, 파일명)`으로 스크린샷을 저장한다.
...
```python
test_actions = [
    ("screenshot", "01_title.png"),         # 타이틀 화면 캡처
    ("key", pygame.K_RETURN),               # Play 선택 → 게임 시작
    ("wait", 0.5),
    ("screenshot", "02_level_start.png"),    # 레벨 1 시작 화면
    ("key", pygame.K_UP),                   # UP 1: 박스를 위로 밀기
    ("wait", 0.3),
    ("screenshot", "03_box_pushed.png"),     # 박스가 (1,2)로 밀린 상태
    ("key", pygame.K_UP),                   # UP 2: 박스를 목표로 밀기 → 클리어!
    ("wait", 0.5),
    ("screenshot", "04_level_clear.png"),    # 클리어 직후 (박스가 목표 위)
    ("wait", 1.0),
    ("screenshot", "05_clear_screen.png"),   # 클리어 결과 화면 + 파티클
]
```

### 구현 요구사항
- `--test` 모드에서도 실제 윈도우가 열리고 모든 렌더링이 보여야 함 (headless 금지)
- 각 키 입력은 `pygame.event.post(pygame.event.Event(KEYDOWN, key=...))` 로 주입
- 대기 시간 동안 게임 루프는 정상 동작 (애니메이션, 파티클 등이 보임)
- 모든 스크린샷 저장 후 자동 종료 (pygame.quit + sys.exit)
- 스크린샷은 PNG 포맷

이 테스트 모드를 통해 AI가 다음을 시각적으로 확인할 수 있다:
1. 타이틀 화면 렌더링 정상 여부
2. 레벨 로드 및 타일맵 표시
3. 플레이어 이동과 박스 밀기
4. 클리어 판정 및 결과 화면
...
코드 작성 완료 후, 아래 절차를 **스스로** 수행하라. 인간 개입 없이 모든 단계를 자율적으로 완료해야 한다.
...
- 에러가 발생하면 트레이스백을 분석하고, 해당 파일을 수정하고, 다시 실행
- 에러가 없을 때까지 반복 (최대 10회)
- 수정 시 다른 모듈과의 인터페이스 정합성을 반드시 확인

### Step 3: 스크린샷 시각 검증
`--test` 실행이 성공하면 `data/screenshots/` 디렉토리의 스크린샷을 **직접 열어서 확인**하라:

```
data/screenshots/01_title.png       → 타이틀 화면이 정상적으로 보이는가? "SOKOBAN" 텍스트와 메뉴가 보이는가?
data/screenshots/02_level_start.png → 벽/바닥/박스/플레이어/목표가 올바르게 렌더링되는가?
data/screenshots/03_box_pushed.png  → 박스가 위로 한 칸 밀렸는가? 플레이어가 박스 아래에 있는가?
data/screenshots/04_level_clear.png → 박스가 목표 위에 있고, 클리어 상태가 감지되었는가?
data/screenshots/05_clear_screen.png → 클리어 결과 화면과 파티클 이펙트가 보이는가?
```

문제가 발견되면 해당 렌더링/로직을 수정하고 `python main.py --test`를 다시 실행하여 재검증.

### Step 4: 기능 검증 체크리스트
스크린샷 검증을 통과한 후, 코드를 읽으며 아래 항목을 추가 확인. 누락이 있으면 즉시 구현:

- [ ] 타이틀 메뉴 5개 항목이 키보드로 선택 가능한가
- [ ] Z키 Undo, Y키 Redo, R키 Restart 로직이 구현되어 있는가
- [ ] ESC 일시정지 메뉴의 Resume/Restart/Level Select/Main Menu가 구현되어 있는가
- [ ] 레벨 선택 화면에서 잠금/해제 로직이 구현되어 있는가
- [ ] 클리어 시 save.json에 기록이 저장되고 다음 레벨이 잠금 해제되는가
- [ ] 에디터에서 타일 배치 + 테스트 플레이가 구현되어 있는가
- [ ] 설정에서 볼륨 조절이 구현되어 있는가
- [ ] 이동 시 lerp 보간이 구현되어 있는가
- [ ] 사운드가 생성/재생되는가

### Step 5: 최종 실행
모든 검증 통과 후 일반 모드로 최종 실행:
```bash
python main.py
```
정상 실행을 확인하고 종료.
...
### 최종 목표 (이것이 달성되면 완료)
> **`python main.py --test` 실행 시 5장의 스크린샷이 `data/screenshots/`에 정상 생성되고, 각 스크린샷에서 타이틀 화면, 레벨 렌더링, 박스 밀기, 클리어 판정, 클리어 결과 화면이 시각적으로 확인되며, `python main.py`로 일반 실행 시 에러 없이 게임이 동작한다.**
